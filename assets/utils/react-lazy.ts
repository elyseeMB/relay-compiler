import { ComponentType, FC, lazy as preactLazy } from "preact/compat";
import { LazyComponent } from "../routes.tsx";

const DEFAULT_STORAGE_KEY = "preactLazyReloadedFunctions";
...
class ReloadStorage {
  ...
  getMap(): Map<string, number> {
    ...
    try {
      ...
      console.error("Error parsing preact-lazy:", error);
      return new Map();
    }
  }
...
export function createLazy<P = {}>(config: LazyConfigInit = {}) {
  ...
  function enhancedLazy<T = P>(
    importFuction: () => Promise<{ default: ComponentType<T> }>
  ): LazyComponent {
    const functionString = importFuction.toString();
